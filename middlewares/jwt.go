...
func NewJWT() *JWT {
	return &JWT{
		[]byte(global.GNA_CONF.JWT.SigningKey),
	}
}
...
func (j *JWT) RefreshToken(accessToken string) (newAccessToken string, err error) {
	...
	// RefreshToken 过期
	if _, err = jwt.Parse(claims.AccessToken, func(token *jwt.Token) (interface{}, error) {
		return j.SigningKey, nil
	}); err != nil {
		return
	}
	...
	if v, _ := parseErr.(*jwt.ValidationError); v.Errors == jwt.ValidationErrorExpired {
		...
		claims.StandardClaims.ExpiresAt = time.Now().Add(time.Hour * time.Duration(global.GNA_CONF.JWT.AccessExpiresTime)).Unix()
		...
		if err != nil {
			if err == TokenExpired {
				...
				if global.GNA_CONF.System.UseMultipoint {
					...
					if err != nil {
						global.GNA_LOG.Error("get redis jwt failed", zap.Any("err", err))
